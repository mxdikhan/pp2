    ...
    if n == 1:
        ...
        for row in table: 
            if name == row[0] and correct_number == True :
                postgre_query = """UPDATE phonebook SET numbers = %s WHERE user_namess = %s """
        ...
        if flag and correct_number == True:
            ...
            postgre_query = """INSERT INTO phonebook( user_namess, numbers) VALUES (%s, %s) """
        ...
        while n >= 1:
            ...
            for row in table:
                if add[0] == row[0] and correct_number == True:
                    postgre_query = """UPDATE phonebook SET numbers = %s WHERE user_names = %s """
            ...
            if flag_mult and correct_number == True:
                data_toinsert = (add[0], add[1])
                postgre_query = """INSERT INTO phonebook( user_names, numbers) VALUES (%s, %s) """
                cursor.execute(postgre_query, data_toinsert)
                conn.commit()
                add_flag = True
    if len(updated) > 0:
...
def delete_user():
    ...
    name = input()
    postgre_query = """DELETE FROM phonebook WHERE user_names = %s """
