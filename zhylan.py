...
account = input()
conn = psycopg2.connect("dbname=postgres user=postgres password=123455")
cursor = conn.cursor()
cursor.execute('SELECT * FROM zhylan LIMIT 10')
...
if add_:
    postgres_insert_query = """ INSERT INTO zhylan (user_name,score,total_score) VALUES (%s,%s,%s)"""
...
size=50
fps=60
bg = pygame.image.load("C:\\Users\\HP\\OneDrive\\Документы\\pp2\\lab8_9\\pictures\\background.jpg")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN :# полностью движение 
            ...
            if event.key == pygame.K_q:
                sql_update_query = """Update zhylan set score = %s where user_name = %s"""
...
total_score+=count
sql_update_query = """Update zhylan set total_score= %s where user_name = %s"""
